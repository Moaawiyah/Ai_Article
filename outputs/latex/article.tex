...
\begin{table}[H]
  ...
  \adjustbox{max width=\textwidth}{
    \begin{tabular}{llll}
      \toprule
      \textbf{Metric} & \textbf{HULA (P4-based)} & \textbf{ECMP (Hardware ASIC)} & \textbf{Software-based LB} \\
      \midrule
      \textbf{Throughput} & Line Rate (100Gbps+) & Line Rate (100Gbps+) & Low (CPU Bound) \\
      \textbf{Latency} & Sub-microsecond (Data Plane) & Sub-microsecond (Data Plane) & Milliseconds (User Space) \\
      \textbf{Consistency} & High (Deterministic) & Low (Non-deterministic) & High (Deterministic) \\
      \textbf{Flexibility} & High (Programmable) & Low (Fixed Hash) & Very High \\
      \textbf{Scalability} & High (Edge) & High (Core) & Low (Bottleneck) \\
      \bottomrule
    \end{tabular}
  }
\end{table}
...
\begin{itemize}
    \item \textbf{Server Registration:} Monitoring the health of backend servers and maintaining a list of active servers.
    \item \textbf{Policy Configuration:} Defining the load balancing algorithm parameters, such as the hash function and the number of hash buckets.
    \item \textbf{Rule Distribution:} Pushing the necessary forwarding rules to the P4-enabled switches via the OpenFlow protocol.
\end{itemize}
...
\subsection{Interaction and Data Flow}
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    node distance=1.8cm,
    client/.style={draw, rectangle, fill=blue!20, minimum width=1.5cm, minimum height=0.8cm, font=\small},
    switch/.style={draw, rectangle, fill=green!20, minimum width=2.5cm, minimum height=0.8cm, font=\small},
    server/.style={draw, rectangle, fill=gray!20, minimum width=1.5cm, minimum height=0.6cm, font=\scriptsize},
    ctrl/.style={draw, circle, fill=red!20, minimum size=1cm, font=\scriptsize},
    edge/.style={-{Stealth[length=4pt]}, thick},
    ctrl_edge/.style={-{Stealth[length=4pt]}, dashed, thick}
  ]
    % Nodes
    \node[client] (c) {Client};
    \node[switch, right=3cm of c] (s) {P4 Switch};
    \node[server, below right=1.5cm and 0.5cm of s] (sv1) {Srv 1};
    \node[server, below left=1.5cm and 0.5cm of s] (sv2) {Srv 2};
    \node[ctrl, above=1.5cm of s] (ctrl) {Controller};

    % Edges
    \draw[edge] (c) -- node[above, font=\tiny]{Data Plane} (s);
    \draw[edge] (s) -- node[above, font=\tiny]{Data Plane} (sv1);
    \draw[edge] (s) -- node[above, font=\tiny]{Data Plane} (sv2);
    \draw[ctrl_edge] (ctrl) -- node[right, font=\tiny]{Control Plane} (s);
  \end{tikzpicture}
  \caption{HULA System Architecture: Data plane forwarding (solid) vs. Control plane signaling (dashed).}
...
\section{Load Balancing Algorithm Design}
...
Where:
\begin{itemize}
    \item $S$ is the ID of the selected server (an integer ranging from 0 to $N-1$).
    \item $H(flow\_tuple)$ is the output of the hash function applied to the packet's header fields.
    \item $N$ is the total number of active backend servers in the cluster.
\end{itemize}
...
\subsection{Consistency and Collision Handling}
...
\subsection{Granularity Trade-offs}
...
\begin{figure}[H]
  ...
  \caption{Left: CDF of bottleneck queue length. Right: average FCT vs.\ network load. Comparison of HULA (P4) vs.\ Software LB vs.\ ECMP (Legacy) (illustrative).}
  \label{fig:perf}
\end{figure}
...
\bibitem{ref1} M. McKeown, N. Anderson, H. Balakrishnan, G. Parulkar, L. Peterson, J. Rexford, S. Shenker, and J. Turner, "OpenFlow: Enabling innovation in campus networks," \textit{ACM SIGCOMM CCR}, vol. 38, no. 2, pp. 69--74, 2008.
\bibitem{ref2} P. Bosshart, D. Daly, G. Varghese, N. McKeown, M. Izzard, F. Kaashoek, N. Sridharan, and W. Voelker, "P4: Programming protocol-independent packet processors," \textit{ACM SIGCOMM CCR}, vol. 44, no. 3, pp. 87--95, 2014.
\bibitem{ref3} M. Al-Fares, S. Radhakrishnan, B. Raghavan, N. Huang, A. Vahdat, and D. A. Katabi, "Hedera: Dynamic packet scheduling for data centers," in \textit{ACM SIGCOMM}, 2010, pp. 89--100.
\bibitem{ref4} N. Foster, R. K. Gupta, J. Rexford, and F. D. Shepherd, "OpenFlow: Experiences with a forwarding control layer," in \textit{ACM HotSDN}, 2011, pp. 3--4.
\bibitem{ref5} S. Shenker, "Foundations of software-defined architectures," in \textit{Proceedings of the 2nd ACM SIGOPS conference on principles of system design}, 2013, pp. 1--14.
\bibitem{ref6} J. C. Mogul, "ECMP: An elegant solution to load balancing in data centers," \textit{ACM Queue}, vol. 10, no. 3, 2012.
\bibitem{ref7} M. Huang, S. Wang, Y. Li, et al., "HULA: Scalable Load Balancing Using Programmable Data Planes," \textit{IEEE INFOCOM}, 2022.
\bibitem{ref8} R. T. Taft, A. C. Polyzos, and M. Varela, "DReMA: A distributed architecture for real-time monitoring and analysis of the Internet," \textit{ACM SIGCOMM CCR}, vol. 32, no. 4, pp. 47--58, 2002.
\bibitem{ref9} A. Greenberg, J. R. Hamilton, N. Jain, S. Kandula, C. Kim, P. Lahiri, D. A. Maltz, P. Zhang, "VL2: A scalable and flexible data center network," in \textit{ACM SIGCOMM}, 2009, pp. 51--62.
\bibitem{ref10} M. Canini, D. Levin, K. H. Lee, L. Petersen, "The Software Defined Networking (SDN) Testbed," in \textit{SIGCOMM Comput. Commun. Rev.}, 2013.
\end{thebibliography}
